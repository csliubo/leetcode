...
class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        def get_one_distance(str):
            ...
            for index, ch in enumerate(str):
                ...
                for i in range(0, 26):
                    ...
                    new_ch = chr(ord('a') + i)
                    if old != new_ch:
                        tmp = tmp[:index] + new_ch + tmp[index + 1:]
                        if tmp in word_set:
                            ret.add(tmp)
                        tmp = str
        ...
        one_degree_words = {}
        # for w in wordList:
        #     one_degree_words[w] = get_one_distance(w)

        min_size = len(wordList)
        nodes = [beginWord]
        from_nodes = {0: [beginWord]}
        i = 0
        distance = {beginWord: 1}
        while i < len(nodes):
            cur = nodes[i]

            path_len = len(from_nodes[i])
            if cur in distance and distance[cur] < path_len:
                i += 1
                continue
            else:
                distance[cur] = path_len
            if path_len > min_size:
                break
            if cur == endWord:
                if not min_size:
                    min_size = path_len
                    self.paths.append(from_nodes[i])
                elif path_len <= min_size:
                    self.paths.append(from_nodes[i])
            if cur not in one_degree_words:
                one_degree_words[cur] = get_one_distance(cur)
            for w in [_ for _ in one_degree_words[cur] if _ not in from_nodes[i]]:
                nodes.append(w)
                from_nodes[len(nodes) - 1] = from_nodes[i] + [w]
...
b = "cet"
e = "ism"
w = ["kid", "tag", "pup", "ail", "tun", "woo", "erg", "luz", "brr", "gay", "sip", "kay", "per", "val", "mes", "ohs",
...
b = "sand"
e = "acne"
w = ["slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock", "fort", "heel", "send",
...
profile.run("s.findLadders(b, e, w)")
