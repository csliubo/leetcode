...
class Solution(object):
    def maxArea(self, height):
        ...
        """
        index = 0
        height_len = height.__len__()
        max_area = 0
        while index < height_len - 1:
            for i in range(index, height_len - 1):
                area = self.area(index + 1, height[index], i + 2, height[i + 1])
                # print area
                if area > max_area:
                    max_area = area
            index += 1
